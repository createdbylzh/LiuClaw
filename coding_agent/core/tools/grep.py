...
from .common import resolve_path, run_shell, truncate_text


def build_grep_tool(workspace_root: Path, cwd: Path, settings: CodingAgentSettings) -> AgentTool:
    ...
    async def execute(arguments: str, context) -> str:
        ...
        search_path = resolve_path(cwd, Path(payload.get("path", ".")))
    ...
    return AgentTool(
        name="grep",
        description="Search text in files on the local filesystem using ripgrep.",
        inputSchema={
            "type": "object",
